...
def host_list():
    ...
    with get_db() as db:
        ...
        if cluster_id:
            ...
            selected_cluster_name = (
                cluster._mapping["cluster_name"] if cluster else None
            )
    ...
    clusters_with_hosts = {}
    for host in hosts:
        cluster_name = host._mapping["cluster_name"] or "Unknown"
        if cluster_name not in clusters_with_hosts:
            clusters_with_hosts[cluster_name] = {
                "location": host._mapping["cluster_location"],
                "hosts": [],
                "total_vms": 0,
                "total_cpus": 0,
                "total_memory_gb": 0,
        ...
        host_dict["vm_count"] = host._mapping["actual_vm_count"]
        clusters_with_hosts[cluster_name]["hosts"].append(host_dict)
        clusters_with_hosts[cluster_name]["total_vms"] += (
            host._mapping["actual_vm_count"] or 0
        )
        clusters_with_hosts[cluster_name]["total_cpus"] += (
            host._mapping["logical_processors"] or 0
        )
        clusters_with_hosts[cluster_name]["total_memory_gb"] += (
            host._mapping["total_memory_gb"] or 0
        )
...
def host_details(host_id):
    ...
    with get_db() as db:
        ...
        vms = db.execute(
            ...
            {"host_name": host._mapping["host_name"]},
        ).fetchall()
